...
if (argv.useDefaultConfig) {
    ...
    filesToCopy.push({from:"src/loader/**/*.html", to:"../assets/loader/",flatten:true});
    filesToCopy.push({context: './src/translate/locales/',from:"**/*.json", to:"../assets/json", force:true});
...
function buildHTMLPluginData (entryNameArray) {
    ...
    entryNameArray.forEach(function (item) {
        retArray.push(new HtmlWebpackPlugin({
            filename: "../" + item.replace(".","").concat(".html"),
            title: buildItem(item),
            app: item,
...
function buildItem(item){
    item=item.replace(".", " ");
    return item.replace(/\b\w/g, function(l){ return l.toUpperCase() });

}

function buildTemplate(item){
    if(item.toLowerCase() === "login" || item.toLowerCase() === "dpos.login" || item.toLowerCase() === "ppv.login" ){
       return path.join(__dirname, "src/login.hbs");
    } else {
         return path.join(__dirname, "src/index.hbs");
    }
}
